...
class player_module:
    ...
    def decision(self,player_data, enemy_data):
        ...
        for data in enemy_data:
            ...
            if not nearest_invader:
                nearest_invader = [dist, data]
            if dist <= nearest_invader[0]:
                if type != 0:
                    nearest_invader = [dist, data]

            if not nearest_UFO:
                nearest_UFO = [dist, data]
            if dist <= nearest_UFO[0]:
                if type == 5:
                    nearest_UFO = [dist, data]
        ...
        if player1_gauge >= 995 and speed != 1. and not release_gauge and gauge_range_enemy_num < 10:
            speed = 1.
            if abs(player1_x-nearest_invader[1][1]) < 0.2:
                tgx = nearest_invader[1][1]
            else:
                tgx = 0.5
            # if UFO exeist
            if nearest_UFO:
                tgx = nearest_UFO[1][1]
            angle = np.arctan2(0.2-player1_y, tgx-player1_x)
